...
class Client:
    ...
    def write(self, sock, path, content):
        ...
        _, body = self.request(
            sock, "file#put", header={"id": id, "name": name}, body=content
        )
        if body != b"OK":
    ...
    def _mvfile(self, sock, id, parent_id, name):
        _, body = self.request(
            sock, "file#mvfile", header={"id": id, "pdid": parent_id, "name": name}
        )
        if body != b"OK":
    ...
    def _mvdir(self, sock, id, parent_id, name):
        _, body = self.request(
            sock, "dir#mvdir", header={"id": id, "pdid": parent_id, "name": name}
        )
        if body != b"OK":
